...
from linformer import LinformerAttentionHead,LinformerMultiHeadAttention
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attention_type = config.get("attention_type", "full")  # "full" or "linformer"

        if self.attention_type == "full":
            if self.use_faster_attention:
                # Fused QKV projection for full attention
            ...
            if self.use_faster_attention:
                ...
                # ★ Original per-head Linformer (slower)
                self.attention = LinformerMultiHeadAttention(config)
